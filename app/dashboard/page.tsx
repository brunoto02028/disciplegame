                    ...
                    <div style={{ position: 'relative' }}>
                        ...
                        {showMenu && (
                            <div style={{ position: 'absolute', right: 0, top: '110%', width: 180, background: '#150d3a', border: goldBorder, borderRadius: 12, padding: '6px 0', zIndex: 100 }}>
                                ...
                                <Link href="/admin/login" style={{ display: 'block', padding: '9px 14px', color: '#c9a227', textDecoration: 'none', fontSize: 13 }}>⚙️ Painel Admin</Link>
                                <hr style={{ border: 'none', borderTop: '1px solid rgba(201,162,39,0.15)', margin: '4px 0' }} />
                ...
                <div className="dash-main-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24, alignItems: 'start' }}>
                    ...
                    <div>
                        ...
                        <div className="dash-stats-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
                            {([
                                { label: 'TOTAL POINTS', value: String(user.stats.totalPoints || 0), sub: 'Lifetime Score', icon: '⭐' },
                                { label: 'GLOBAL RANK', value: '#—', sub: 'Ranking Global', icon: '🏆' },
                                { label: 'ACCURACY', value: `${user.stats.avgAccuracy || 0}%`, sub: 'Ultimas partidas', icon: '🎯' },
                                { label: 'CITIES UNLOCKED', value: `${completedCount}/${totalCities}`, sub: 'Pilgrimage Progress', icon: '📍' },
                        ...
                        <div style={{ ...glass, padding: 24 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                                <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.8)', fontFamily: "'Playfair Display','Georgia',serif" }}>PILGRIMAGE PATH</span>
                                <span style={{ fontSize: 11, color: '#c9a227', background: 'rgba(201,162,39,0.1)', border: goldBorder, borderRadius: 20, padding: '3px 10px' }}>✝ MVP Inicial</span>
                            </div>
                            {/* Progress bar */}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                        ...
                        <div style={{ ...glass, padding: 20 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                                <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.7)', fontFamily: "'Playfair Display','Georgia',serif" }}>TOP 5 RANKINGS</p>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                {rankings.length > 0 ? rankings.map((player, i) => (
                                    <div key={player.rank} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: i < 3 ? '10px 12px' : '8px 12px', borderRadius: 10, background: i < 3 ? 'rgba(201,162,39,0.06)' : 'transparent', border: i < 3 ? goldBorder : '1px solid transparent' }}>
                                        <div style={{ width: 30, height: 30, borderRadius: '50%', background: i < 3 ? rankMedals[i] : 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, flexShrink: 0, color: i < 3 ? '#1a0a4a' : '#fff' }}>
                                            {player.rank}
                                        </div>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <p style={{ fontWeight: 600, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{player.user_name}</p>
                                            <p style={{ fontSize: 11, color: '#c9a227', fontWeight: 600 }}>{player.total_points} Pts.</p>
                                        </div>
                                        {i === 0 && <span style={{ fontSize: 16 }}>👑</span>}
                                        {i === 1 && <span style={{ fontSize: 16 }}>🛡️</span>}
                                        {i === 2 && <span style={{ fontSize: 16 }}>🏅</span>}
                                    </div>
                                )) : (
                                    <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', textAlign: 'center', padding: '16px 0' }}>Seja o primeiro no ranking!</p>
                                )}
                            </div>
                        </div>

                        {/* Weekly Challenge */}
                        <div style={{ background: 'linear-gradient(135deg,rgba(201,162,39,0.08),rgba(26,10,74,0.9))', border: goldBorder, borderRadius: 16, padding: 20, position: 'relative', overflow: 'hidden' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#c9a227,#8b6914)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <CrossIcon size={16} color="#fff" />
                                </div>
                                <div>
                                    <p style={{ fontSize: 10, fontWeight: 700, color: '#c9a227', letterSpacing: 1.2 }}>DESAFIO SEMANAL</p>
                                    <p style={{ fontSize: 14, fontWeight: 700, fontFamily: "'Playfair Display','Georgia',serif" }}>Parabolas da Luz</p>
                                </div>
                            </div>
                            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6, marginBottom: 16 }}>
                                Complete {cities[0]?.name || 'uma cidade'} com 100% de acertos.<br />
                                Recompensa: <strong style={{ color: '#c9a227' }}>500 pts bonus</strong>
                            </p>
                            {cities.length > 0 && (
                                <Link href={`/game/${cities[0].id}`} style={{ display: 'block', width: '100%', padding: '11px', borderRadius: 10, background: 'linear-gradient(135deg,#c9a227,#8b6914)', color: '#1a0a4a', fontWeight: 700, fontSize: 13, textDecoration: 'none', textAlign: 'center', letterSpacing: 0.5 }}>
                                    ✝ ACEITAR DESAFIO
                                </Link>
                            )}
                            <WeeklyCountdown />
